...
function convertMatrixToByteArray(matrix: number[]): number[] {
    ...
    // Parcourir la matrice par blocs de 8 bits
    for (let i = 0; i < matrix.length; i += 8) {
      let byte = 0;
  
      // Convertir chaque bloc de 8 bits en un octet
      for (let j = 0; j < 8; j++) {
        byte <<= 1; // Décaler les bits vers la gauche
        byte |= matrix[i + j] ? 1 : 0; // Ajouter le bit actuel
      }
  
      byteArray.push(byte); // Ajouter l'octet au tableau
    }
...
const BluetoothImageSender = () => {
  ...
  const createSimpleImageBuffers = (mat: number[]): Uint8Array[] => {
    ...
    return [
      new Uint8Array([0x1B, 0x00, ...byteArray.slice(0, 168)]),   // First buffer
      new Uint8Array([0x1B, 0x01, ...byteArray.slice(168, 336)]), // Second buffer
      new Uint8Array([0x1B, 0x02, ...byteArray.slice(336, 504)])  // Third buffer
    ];
  };
  ...
  const getCharactersMatrice = async (): Promise<number[]> => {
    try {
      ...
      // Récupérer la matrice sous forme de chaîne hexadécimale
      const hexMatrice = response.data[0].matrice;
  
      // Convertir la chaîne hexadécimale en tableau de bits
      const bitArray: number[] = [];
      for (let i = 0; i < hexMatrice.length; i += 2) {
        const byte = parseInt(hexMatrice.substr(i, 2), 16); // Convertir 2 caractères hex en un octet
        for (let j = 7; j >= 0; j--) {
          bitArray.push((byte >> j) & 1); // Extraire chaque bit et l'ajouter au tableau
        }
      }
  ...
  const startScan = () => {
    ...
    bleManager.startDeviceScan(null, null, async (error, device) => {
      ...
      if (device?.name === "SportsLab_Tamagochi") {
        ...
        try {
          // Récupérer la matrice avant de se connecter
  ...
  const sendImageToScreen = async () => {
    ...
    try {
      // Créer les buffers avec la matrice récupérée
